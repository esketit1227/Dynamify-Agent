...
export const demoStrategyOutput = z.object({
  targetPageUrl: z.string(),
  targetSegment: z.string(),
  narrativeSummary: z.string(),
  preserveElements: z.array(z.string()).min(1),
  plannedChanges: z.array(plannedChange).min(1).max(8),
});
export type DemoStrategyOutput = z.infer<typeof demoStrategyOutput>;

// ---------------------------------------------------------------------------
// Preview generation — a bounded set of targeted DOM patches, not a full
// HTML rewrite. This is what makes the generated preview preserve the
// prospect's design language (logo, typography, color system, layout): the
// model never touches markup or styles it wasn't asked to change, it only
// edits the specific elements identified by CSS selector.
// ---------------------------------------------------------------------------

export const domPatchOperation = z.object({
  /** CSS selector for the element to change. Only the first match is used. */
  selector: z.string().min(1),
  /** Which planned change (by section) this patch implements. */
  section: z.string(),
  action: z.enum(["set_text", "set_html", "set_attribute", "remove"]),
  /** New text/inner-HTML/attribute value. Required for set_text/set_html/set_attribute, omitted for remove. */
  value: z.string().nullable(),
  /** Required when action is "set_attribute" (e.g. "href", "src", "alt"). */
  attribute: z.string().nullable(),
});
export type DomPatchOperation = z.infer<typeof domPatchOperation>;

export const previewGenerationOutput = z.object({
  patches: z.array(domPatchOperation).min(1).max(30),
  changesSummary: z.array(plannedChange).min(1),
  preservedElementsConfirmed: z.array(z.string()),
});
export type PreviewGenerationOutput = z.infer<typeof previewGenerationOutput>;

// ---------------------------------------------------------------------------
// Preview QA
// ---------------------------------------------------------------------------

export const previewQaCheckResult = z.object({
  checkType: z.enum([
    "brand_consistency",
    "factual_accuracy",
    "broken_link",
...
export const previewQaOutput = z.object({
  checks: z.array(previewQaCheckResult).min(1),
  overallPassed: z.boolean(),
});
export type PreviewQaOutput = z.infer<typeof previewQaOutput>;
